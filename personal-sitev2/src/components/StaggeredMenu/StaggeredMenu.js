...
export const StaggeredMenu = ({
  ...
  React.useEffect(() => {
    if (toggleBtnRef.current) {
      ...
      gsap.set(toggleBtnRef.current, { color: changeMenuColorOnOpen ? targetColor : menuButtonColor });
    }
  ...
  const handleItemClick = useCallback(
    (item) => {
      ...
      // external links: default <a> behavior, but we still close if we render Link (we don't for external)
    },
    [navigate, closeMenu]
  );
      ...
      <header className="staggered-menu-header" aria-label="Main navigation header">
        <Link to="/" className="sm-logo" aria-label="Logo" onClick={closeMenu}>
          {logoUrl ? (
            ...
            <span className="sm-logo-text">ONUR GUL</span>
          )}
        </Link>
        <button
          ref={toggleBtnRef}
              ...
              items.map((it, idx) => (
                <li className="sm-panel-itemWrap" key={it.label + idx}>
                  {isInternalLink(it.link) ? (
                    ...
                    >
                      {displayItemNumbering && <span className="sm-panel-itemNum" style={{ opacity: 'var(--sm-num-opacity, 1)' }}>{String(idx + 1).padStart(2, '0')}</span>}
